...
cnt = len(li_elements)
while cnt > 0:
    tapd = li_elements[0].find_element(by=By.CLASS_NAME, value='phui-oi-name').find_element(by=By.TAG_NAME, value='a').text
    auditors = li_elements[0].find_element(by=By.CLASS_NAME, value='phui-oi-content').find_element(by=By.TAG_NAME, value='a').text
    print(auditors + '---' + tapd)
    # 审核人不是自己
    if auditors.count(username) == 0:
        continue
    li_elements[0].find_element(by=By.CLASS_NAME, value='phui-oi-name').find_element(by=By.TAG_NAME, value='a').click()
